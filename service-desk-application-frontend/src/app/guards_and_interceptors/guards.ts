...
export const userIsEmployeeOrAnalyst: CanActivateFn = (route: ActivatedRouteSnapshot, state: RouterStateSnapshot) => {
  const userData: AuthData | null = inject(AuthService).userLogged;
  if(userData === null || userData.roles.filter(element => element === "EMPLOYEE" || element === "FIRST_LINE_ANALYST" || element === "SECOND_LINE_ANALYST").length < 1)
    return inject(AuthService).redirectUser();
